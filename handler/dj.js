async function dj(bot, member, song, guildId) {
    ...
    if (!bot)
        return false;
    ///// 
    let data = await Guild.findOne({ guildID: guildId }); //get the adminroles
    if (!data)
        return;
    const roleid = data.plugins.music.dj;
    ...
    for (let i = 0; i < roleid.length; i++) {
        ...
        if (!member.guild.roles.cache.get(roleid[i]))
            continue;
        //if he has role set var to true
        if (member.roles.cache.has(roleid[i]))
            isdj = true;
